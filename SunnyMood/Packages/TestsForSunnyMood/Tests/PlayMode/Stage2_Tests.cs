...
[Description("The glorious lamp of heaven, the Sun."), Category("2")]
public class Stage2_Tests
{
    ...
    public IEnumerator MovementLeftCheck()
    {
        ...
        {
            Assert.Fail("Level 1: When the A-key is pressed \"Player\"'s position should change");
        }
    ...
    public IEnumerator MovementRightCheck()
    {
        ...
        if (posStart == playerT.position)
        {
            Assert.Fail("Level 1: When the D-key is pressed \"Player\"'s position should change");
        }
    ...
    public IEnumerator JumpCheck()
    {
        ...
        {
            Assert.Fail(
                "Level 1: Y-axis velocity of <Rigidbody2D> component should decrease after the jump was provided," +
    ...
    public IEnumerator CheckBounds()
    {
        ...
        foreach (GameObject b in bounds)
        {
            ...
            if (boundsSR)
                Assert.Fail(
                    "Level 1: There should be no <SpriteRenderer> component on \"Bounds\" objects in order to make them non-visible");
            ...
            {
                Assert.Fail(
                    "Level 1: \"Bounds\"' <Collider2D> component should have assigned <2D Physics Material> with " +
        ...
        {
            Assert.Fail(
                "Level 1: There should be an object with \"Bounds\" tag on \"Player\"'s right and stop it from" +
                " passing through (\"Player\" should not be able to jump it over, or crawl underneath it)");
        }
        ...
        {
            Assert.Fail(
                "Level 1: Left boundary should be placed above or 'touching' the ground object, so \"Player\" won't" +
        ...
        {
            Assert.Fail(
                "Level 1: Right boundary should be placed above or 'touching' the ground object, so \"Player\" won't" +
                " be able to fall down to the void");
        }
    }
    ...
    public IEnumerator CameraMovementCheck()
    {
        ...
        {
            Assert.Fail(
                "Level 1: Player should be able to get from left bound to the right one in less than 10 game-time seconds");
        }
        VInput.KeyUp(KeyCode.A);
    ...
    public IEnumerator LevelEndCheck()
    {
        ...
        {
            Assert.Fail(
                "Level 1: \"LevelEnd\" should be placed above or 'touching' the ground object, so \"Player\" could reach it");
        }
        ...
        {
            Assert.Fail(
                "Level 1: When \"Player\" collides with a \"LevelEnd\" object - scene should change (or just reload).");
        }
    }
}