...
public class Stage4_3_Tests
{
    ...
    public IEnumerator NecessaryComponents()
    {
        ...
        {
            Assert.Fail(
                " Level 2: Player should have assigned enabled <Animator> component in order to perform animations");
        }
    ...
    public IEnumerator CheckAnimationClips()
    {
        ...
        if (idle.legacy)
            Assert.Fail(" Level 2: \"Idle\" clip should be animated by animator, not by the <Legacy Animation>" +
        ...
        if (jump.legacy)
            Assert.Fail(" Level 2: \"Jump\" clip should be animated by animator, not by the <Legacy Animation>" +
                        " component, so it's legacy property should be unchecked");
        ...
        if (walk.legacy)
            Assert.Fail(" Level 2: \"Walk\" clip should be animated by animator, not by the <Legacy Animation>" +
                        " component, so it's legacy property should be unchecked");
    ...
    public IEnumerator CheckTransitions()
    {
        ...
        {
            Assert.Fail(
                " Level 2: While character is moving on the ground to the right - \"Walk\" clip should be played");
        }
        ...
        {
            Assert.Fail(
                " Level 2: While character is moving on the ground to the left - \"Walk\" clip should be played");
        }

        if (!sr.flipX)
        {
            Assert.Fail(
                " Level 2: If last character's movement was performed to the left - it's sprite should be flipped");
        }
    ...
    public IEnumerator CheckGemAnimationClips()
    {
        ...
        if (gemClip.legacy)
            Assert.Fail(" Level 2: \"Gem\" clip should be animated by animator, not by the <Legacy Animation>" +
                        " component, so it's legacy property should be unchecked");
