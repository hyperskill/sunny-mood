...
public class Stage4_2_Tests
{
    ...
    public IEnumerator NotMovingWithoutInputCheck()
    {
        ...
        {
            Assert.Fail("\"Level 2\" scene is misspelled or was not added to build settings");
        }
        ...
        yield return new WaitUntil(() =>
            SceneManager.GetActiveScene().name == "Level 2" || (Time.unscaledTime - start) * Time.timeScale > 1);
        if (SceneManager.GetActiveScene().name != "Level 2")
        {
            Assert.Fail("\"Level 2\" scene can't be loaded");
        }
    ...
    public IEnumerator MovementLeftCheck()
    {
        ...
        {
            Assert.Fail("Level 2: When the A-key is pressed \"Player\"'s position should change");
        }
    ...
    public IEnumerator MovementRightCheck()
    {
        ...
        if (posStart == playerT.position)
        {
            Assert.Fail("Level 2: When the D-key is pressed \"Player\"'s position should change");
        }
