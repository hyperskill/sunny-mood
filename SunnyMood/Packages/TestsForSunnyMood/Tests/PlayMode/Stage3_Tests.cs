...
public class Stage3_Tests
{
    ...
    public IEnumerator NecessaryComponents()
    {
        ...
        {
            Assert.Fail(
                " Level 1: Player should have assigned enabled <Animator> component in order to perform animations");
        }
    ...
    public IEnumerator CheckTransitions()
    {
        ...
        {
            Assert.Fail(
                " Level 1: While character is moving on the ground to the right - \"Walk\" clip should be played");
        }
        ...
        {
            Assert.Fail(
                " Level 1: While character is moving on the ground to the left - \"Walk\" clip should be played");
        }

        if (!sr.flipX)
        {
            Assert.Fail(
                " Level 1: If last character's movement was performed to the left - it's sprite should be flipped");
        }
    ...
    public IEnumerator CheckGemAnimationClips()
    {
        ...
        if (gemClip.legacy)
            Assert.Fail(" Level 1: \"Gem\" clip should be animated by animator, not by the <Legacy Animation>" +
                        " component, so it's legacy property should be unchecked");
