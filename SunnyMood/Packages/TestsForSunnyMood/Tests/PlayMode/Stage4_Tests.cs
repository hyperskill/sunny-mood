...
[Description("When the Sun has set, no candle can replace it."), Category("4")]
public class Stage4_Tests
{
    ...
    public IEnumerator NecessaryComponents()
    {
        ...
        foreach (GameObject platform in platforms)
        {
            ...
            {
                Assert.Fail(
                    "Level 2: Platforms should be children of \"Grid\" object, because tilemap should be a child of grid");
            }
            ...
            if (!platformTMR)
                Assert.Fail(
                    "Level 2: There should be a <TilemapRenderer> component on \"Platform\"s' object to view created tilemaps");
            ...
            {
                Assert.Fail(
                    "Level 2: \"Platform\"s' <Collider2D> component should have assigned <2D Physics Material> with " +
            ...
            {
                Assert.Fail(
                    "Level 2: Let \"Platform\"s' <Collider2D> component be at least twice as wider, than player's one");
            }
        }
    }
    ...
    public IEnumerator CheckCollisions()
    {
        ...
        yield return null;
        Time.timeScale = 1;
        groundCl.gameObject.layer = LayerMask.NameToLayer("Ground");
