...
public class B_MovementTest
{
    ...
    public IEnumerator NotMovingWithoutInputCheck()
    {
        ...
        {
            Assert.Fail("Level 1: In some time after the scene was loaded \"Player\"'s collider should be \"touching\" \"Ground\"'s collider" +
        ...
        {
            Assert.Fail("Level 1: \"Player\"'s position should not change if there were no input provided");
        }
    }
    [UnityTest, Order(1)]
    public IEnumerator MovementLeftCheck()
    {
        ...
        {
            Assert.Fail("Level 1: When the A-key is pressed X-axis of \"Player\"'s object should decrease");
        }

        if (posEnd.y != posStart.y || posEnd.z != posStart.z)
        {
            Assert.Fail("Level 1: When the A-key is pressed y-axis and z-axis should not change");
        }
        yield return null;
    }
    ...
    public IEnumerator MovementRightCheck()
    {
        ...
        if (posEnd.x <= posStart.x)
        {
            Assert.Fail("Level 1: When the D-key is pressed X-axis of \"Player\"'s object should increase");
        }

        if (posEnd.y != posStart.y || posEnd.z != posStart.z)
        {
            Assert.Fail("Level 1: When the D-key is pressed y-axis and z-axis should not change");
        }
        yield return null;
    }
    ...
    public IEnumerator CorrectMovement()
    {
        ...
        if (posEnd.x != posStart.x)
        {
            Assert.Fail("Level 1: \"Player\"'s movement should be implemented via <Rigidbody2D> component");
        }
        ...
        {
            Assert.Fail("Level 1: \"Player\"'s horizontal movement should be implemented by changing x-axis velocity");
        }
        IS.Keyboard.KeyUp(VirtualKeyCode.VK_D);
        yield return null;
    }
    ...
    public IEnumerator JumpCheck()
    {
        ...
        if ((Time.unscaledTime - start) * Time.timeScale >= 5)
        {
            Assert.Fail("Level 1: When the Space-button was pressed jump should be provided, so the Rigidbody2D's " +
        ...
        if ((Time.unscaledTime - start) * Time.timeScale >= 5)
        {
            Assert.Fail("Level 1: Y-axis velocity of <Rigidbody2D> component should decrease after the jump was provided," +
        ...
        if (duration >= 2)
        {
            Assert.Fail("Level 1: Jump duration should be less than 2 seconds, but in your case it's "+duration);
        }
    }
    ...
    public IEnumerator JumpInAirCheck()
    {
        ...
        {
            Assert.Fail("Level 1: \"Player\" should not be able to jump while it's in midair");
        }
    ...
    public IEnumerator CheckBounds()
    {
        ...
        for (int i = 0; i < tagsProp.arraySize; i++)
        {
            ...
            if (t.stringValue.Equals("Bounds")) { BoundsTagExist = true; break; }
        }

        if (!BoundsTagExist)
        {
            Assert.Fail("Level 1: \"Bounds\" tag was not added to project");
        }
        ...
        if (bounds.Length != 2)
        {
            Assert.Fail("Level 1: There should be 2 game objects with \"Bounds\" tag on scene");
        }
        ...
        foreach (GameObject b in bounds)
        {
            ...
            if(boundsSR)
                Assert.Fail("Level 1: There should be no <SpriteRenderer> component on \"Bounds\" objects in order to make them non-visible");
            Collider2D boundsCL = PMHelper.Exist<Collider2D>(b);
            if (!boundsCL || !boundsCL)
            {
                Assert.Fail("Level 1: \"Bounds\" objects should have an assigned enabled <Collider2D> component");
            }
            if (boundsCL.isTrigger)
            {
                Assert.Fail("Level 1: \"Bounds\"' <Collider2D> component should not be triggerable");
            }

            if (!boundsCL.sharedMaterial || boundsCL.sharedMaterial.friction != 0)
            {
                Assert.Fail("Level 1: \"Bounds\"' <Collider2D> component should have assigned <2D Physics Material> with " +
        ...
        if (!leftBoundCl)
        {
            Assert.Fail("Level 1: There should be an object with \"Bounds\" tag on \"Player\"'s left and stop it from" +
                        " passing through (\"Player\" should not be able to jump it over, or crawl underneath it)");
        }
        if (!rightBoundCl)
        {
            Assert.Fail("Level 1: There should be an object with \"Bounds\" tag on \"Player\"'s right and stop it from" +
                        " passing through (\"Player\" should not be able to jump it over, or crawl underneath it)");
        }
        ...
        if (!PMHelper.RaycastFront2D(leftBoundPoint, Vector2.down,
            1 << LayerMask.NameToLayer("Test")).collider)
        {
            Assert.Fail("Level 1: Left boundary should be placed above or 'touching' the ground object, so \"Player\" won't" +
                        " be able to fall down to the void");
        }
        if (!PMHelper.RaycastFront2D(rightBoundPoint, Vector2.down,
            1 << LayerMask.NameToLayer("Test")).collider)
        {
            Assert.Fail("Level 1: Right boundary should be placed above or 'touching' the ground object, so \"Player\" won't" +
                        " be able to fall down to the void");
        }
    }
    ...
    {
        GameObject cameraObj = PMHelper.Exist("Main Camera");
        ...
        {
            Assert.Fail("Level 1: Player should be able to get from left bound to the right one in less than 10 game-time seconds");
        }
        IS.Keyboard.KeyUp(VirtualKeyCode.VK_A);
        yield return null;
        ...
        {
            Assert.Fail("Level 1: Player should always stay in a camera view");
        }
    ...
    public IEnumerator LevelEndPositionCheck()
    {
        ...
        if (!PMHelper.RaycastFront2D(LevelEndPlace, Vector2.down,
            1 << LayerMask.NameToLayer("Test")).collider)
        {
            Assert.Fail("Level 1: \"LevelEnd\" should be placed above or 'touching' the ground object, so \"Player\" could reach it");
        }
        
        if (LevelEndPlace.x <= leftBoundPoint.x || LevelEndPlace.x >= rightBoundPoint.x)
        {
            Assert.Fail("Level 1: \"LevelEnd\" should be placed between boundaries, so \"Player\" could reach it");
        }

        if (LevelEndPlace.y >= jumpPlace.y)
        {
            Assert.Fail("Level 1: \"LevelEnd\" should not be placed too high, so \"Player\" could reach it");
        }
    }
    ...
    public IEnumerator LevelEndActionCheck()
    {
        ...
        if ((Time.unscaledTime - start) * Time.timeScale >= 5)
        {
            Assert.Fail("Level 1: When \"Player\" collides with a \"LevelEnd\" object - scene should change (or just reload).");
        }
    }
}