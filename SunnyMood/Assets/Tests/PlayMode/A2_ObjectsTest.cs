...
public class A2_ObjectsTest
{
    ...
    public IEnumerator BasicObjectComponentsCheck()
    {
        ...
        {
            Assert.Fail("Level 2: Player's <Rigidbody2D> component's constraints should be freezed by rotation and unfreezed by position");
        }
    ...
    public IEnumerator BasicGridComponentsCheck()
    {
        ...
        if(!groundTMR)
            Assert.Fail("Level 2: There should be a <TilemapRenderer> component on \"Ground\"'s object to view created tilemaps");
    ...
    public IEnumerator ChildrenCheck()
    {
        ...
        if(!PMHelper.Child(background,cameraObj))
            Assert.Fail("Level 2: \"Background\"'s object should be a child of \"Main Camera\" object in order to move" +
                        " camera with background, so that the background will be always in the camera view");
        if(!PMHelper.Child(ground,grid))
            Assert.Fail("Level 2: \"Ground\"'s object should be a child of \"Grid\" object, because tilemap's should be a child of grid!");
    }
    ...
    public IEnumerator SortingCheck()
    {
        ...
        if(!correctSort)
            Assert.Fail("Level 2: Order in layers should be placed in correct order: Background < LevelEnd < Ground < Player!");
    }
