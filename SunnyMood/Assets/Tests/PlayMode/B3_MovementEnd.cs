...
public class B3_MovementTest
{
    ...
    public IEnumerator JumpCheck()
    {
        ...
        if ((Time.unscaledTime - start) * Time.timeScale >= 5)
        {
            Assert.Fail("Level 3: Y-axis velocity of <Rigidbody2D> component should decrease after the jump was provided," +
    ...
    public IEnumerator CheckBounds()
    {
        ...
        foreach (GameObject b in bounds)
        {
            ...
            if(boundsSR)
                Assert.Fail("Level 3: There should be no <SpriteRenderer> component on \"Bounds\" objects in order to make them non-visible");
            ...
            {
                Assert.Fail("Level 3: \"Bounds\"' <Collider2D> component should have assigned <2D Physics Material> with " +
        ...
        if (!rightBoundCl)
        {
            Assert.Fail("Level 3: There should be an object with \"Bounds\" tag on \"Player\"'s right and stop it from" +
                        " passing through (\"Player\" should not be able to jump it over, or crawl underneath it)");
        }
        ...
        if (!PMHelper.RaycastFront2D(leftBoundPoint, Vector2.down,
            1 << LayerMask.NameToLayer("Test")).collider)
        {
            Assert.Fail("Level 3: Left boundary should be placed above or 'touching' the ground object, so \"Player\" won't" +
                        " be able to fall down to the void");
        }
        if (!PMHelper.RaycastFront2D(rightBoundPoint, Vector2.down,
            1 << LayerMask.NameToLayer("Test")).collider)
        {
            Assert.Fail("Level 3: Right boundary should be placed above or 'touching' the ground object, so \"Player\" won't" +
                        " be able to fall down to the void");
        }
    }
    ...
    public IEnumerator CameraMovementCheck()
    {
        ...
        {
            Assert.Fail("Level 3: Player should be able to get from left bound to the right one in less than 10 game-time seconds");
        }
        IS.Keyboard.KeyUp(VirtualKeyCode.VK_A);
        yield return null;
    ...
    public IEnumerator LevelEndPositionCheck()
    {
        ...
        if (!PMHelper.RaycastFront2D(LevelEndPlace, Vector2.down,
            1 << LayerMask.NameToLayer("Test")).collider)
        {
            Assert.Fail("Level 3: \"LevelEnd\" should be placed above or 'touching' the ground object, so \"Player\" could reach it");
        }
    ...
    public IEnumerator LevelEndActionCheck()
    {
        ...
        if ((Time.unscaledTime - start) * Time.timeScale >= 5)
        {
            Assert.Fail("Level 3: When \"Player\" collides with a \"LevelEnd\" object - scene should change (or just reload).");
        }
    }
}