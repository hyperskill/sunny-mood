...
public class C_Animations_Test
{
    ...
    public IEnumerator NecessaryComponents()
    {
        ...
        {
            Assert.Fail(" Level 1: Player should have assigned enabled <Animator> component in order to perform animations");
        }
        if (!anim.runtimeAnimatorController)
        {
            Assert.Fail(" Level 1: There should be created controller, attached to <Animator> component");
        }
    ...
    public IEnumerator CheckAnimationClips()
    {
        ...
        if (idle.legacy) Assert.Fail(" Level 1: \"Idle\" clip should be animated by animator, not by the <Legacy Animation>" +
        ...
        if (jump.legacy) Assert.Fail(" Level 1: \"Jump\" clip should be animated by animator, not by the <Legacy Animation>" +
                                     " component, so it's legacy property should be unchecked");
        ...
        if (walk.legacy) Assert.Fail(" Level 1: \"Walk\" clip should be animated by animator, not by the <Legacy Animation>" +
                                     " component, so it's legacy property should be unchecked");
    ...
    public IEnumerator CheckTransitions()
    {
        ...
        {
            Assert.Fail(" Level 1: While character is not moving - \"Idle\" clip should be played");
        }
        ...
        if ((Time.unscaledTime - start) * Time.timeScale >= 2)
        {
            Assert.Fail(" Level 1: While character is moving on the ground to the right - \"Walk\" clip should be played");
        }
        ...
        if ((Time.unscaledTime - start) * Time.timeScale >= 2)
        {
            Assert.Fail(" Level 1: While character is moving on the ground to the left - \"Walk\" clip should be played");
        }
        
        if (!sr.flipX)
        {
            Assert.Fail(" Level 1: If last character's movement was performed to the left - it's sprite should be flipped");
        }
    ...
    public IEnumerator NecessaryGemComponents()
    {
        ...
        if(!gemSR||!gemSR.enabled)
            Assert.Fail(" Level 1: There is no <SpriteRenderer> component on \"Gem\" or it is disabled");
        if(!gemSR.sprite)
        ...
        {
            Assert.Fail(" Level 1: \"Gem\"'s order in layer should be greater than \"Background\"'s one");
        }

        gemCL = PMHelper.Exist<Collider2D>(gem);
        if (!gemCL || !gemCL.enabled)
        {
            Assert.Fail(" Level 1: \"Gem\" objects should have assigned enabled <Collider2D> component");
        }
        if (!gemCL.isTrigger)
        {
    ...
    public IEnumerator CheckGemAnimationClips()
    {
        ...
        if (gemClip.legacy) Assert.Fail(" Level 1: \"Gem\" clip should be animated by animator, not by the <Legacy Animation>" +
                                        " component, so it's legacy property should be unchecked");
        if (gemClip.empty) Assert.Fail(" Level 1: \"Gem\" clip in Gem's animator should have animation keys");
        if (!gemClip.isLooping) Assert.Fail(" Level 1: \"Gem\" clip in Gem's animator should be looped");
    }
    ...
    public IEnumerator CheckTransitionsIdle()
    {
        ...
        if ((Time.unscaledTime - start) * Time.timeScale >= 2)
        {
            Assert.Fail(" Level 1: \"Gem\" clip should be played by default");
        }
    }
